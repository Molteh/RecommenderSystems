...
from Base.Similarity.Cython.Compute_Similarity_Cython import Compute_Similarity_Cython as Cython_Cosine_Similarity
...
class Utils(object):
    ...
    def get_URM_seq(self):
        URM_seq = self.URM[self.target_playlists[:5000]]

    def build_URM_Random(self):
        random = self.train[self.train["playlist_id"] not in self.target_playlists[:5000]]
        grouped = self.random.groupby('playlist_id', as_index=True).apply((lambda playlist: list(playlist['track_id'])))
        URM = MultiLabelBinarizer(classes=self.tracks['track_id'].unique(), sparse_output=True).fit_transform(grouped)
        return URM.tocsr()
