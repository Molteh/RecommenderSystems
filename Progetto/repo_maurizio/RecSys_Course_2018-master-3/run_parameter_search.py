...
def run_EnsembleRecommender_on_similarity_type(parameterSearch, URM_train, n_cases, output_root_path, metric_to_optimize):
...
def run_KNNCFRecommender_on_similarity_type(similarity_type, parameterSearch, URM_train, n_cases, output_root_path, metric_to_optimize):
    ...
    hyperparamethers_range_dictionary["shrink"] = [0, 5, 10, 20, 50, 100, 200, 300, 500, 1000]
...
def runParameterSearch_Collaborative(recommender_class, URM_train, metric_to_optimize = "PRECISION",
    ...
    try:
        ...
        if recommender_class is ItemKNNCFRecommender:
            ...
            similarity_type_list = ['cosine', 'jaccard', "asymmetric", "dice", "tversky"]
            #similarity_type_list = ["asymmetric", "dice", "tversky"]
        ...
        if recommender_class is EnsembleRecommender:
            ...
            '''run_KNNCFRecommender_on_similarity_type_partial = partial(run_EnsembleRecommender_on_similarity_type,
                                                                      parameterSearch=parameterSearch,
                                                                      URM_train=URM_train,
                                                                      n_cases=n_cases,
                                                                      output_root_path=output_root_path_rec_name,
            ...
            if parallelizeKNN:
                pool = PoolWithSubprocess(processes=int(2), maxtasksperchild=1)
                resultList = pool.map(run_KNNCFRecommender_on_similarity_type_partial)

            else:
                run_EnsembleRecommender_on_similarity_type()
            return'''
...
def read_data_split_and_search():
    ...
    def split_train_validation_test(URM_all, split_train_test_validation_quota):
        ...
        URM_test = sps.coo_matrix((URM_all.data[testMask], (URM_all.row[testMask], URM_all.col[testMask])),
                                  shape=URM_shape)
    ...
    #URM_train, URM_validation, URM_test = split_train_validation_test(u.get_URM(), [0.6, 0.2, 0.2])
