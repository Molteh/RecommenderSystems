...
class SimilarityMatrixRecommender(object):
    ...
    def compute_score_ensemble_based(self, user_id):

        if self.sparse_weights:

            user_profile = self.URM_train[user_id]

            score = (self.S_UCF[user_id].dot(self.URM_train).toarray() +
                    user_profile.dot(self.S_ICF).toarray())
    ...
    #If recommender is ensemble save S matrixes

    def saveModel(self, folder_path, file_name = None):

        print(folder_path)

        if folder_path=="result_experiments/EnsembleRecommender":
            print("true")
            ...
            if self.sparse_weights:
                dictionary_to_save["S_ICF"] = self.S_ICF
                dictionary_to_save["S_UCF"] = self.S_UCF
