...
class MFBPR(object):
    ...
    def recommend(self, target_playlist):
        rec = np.dot(self.user_factors[target_playlist], self.item_factors.T)
        rec_csr = sparse.csr_matrix(rec)
        rec_csr = self.u.get_top_10(self.URM, target_playlist, rec_csr.toarray().ravel())
        return rec_csr
